...
## 145060 Programming · Unit 5 · Due Week 11, Friday

**Mode:** solo. **Gate:** 3, full tooling, decision log and AI usage log required.
**Periods:** Build 2 on Thursday of Week 11 and a 30-minute pipeline block on Friday of Week 11, plus any
time you have after your text adventure v3 is submitted.

**This is the shortest project in the course: about seventy minutes of class time.** It fits because most of it is assembly.
Your Lab U5-05 scraper already collected the listings, your Lab U5-04 cleaner already shows the cleaning
pattern, and your Lab U5-02 report already shows the calculated fields. Finish those labs first if they
are not done, because they are the fastest way through this project.
...
You choose **one** of two routes by the end of Thursday's Build 2.

### Route A: the treasurer's data (recommended)

- **The listings:** the 28 listings your own Lab U5-05 scraper collected from the Swap Shelf website,
  `05-labs/fixtures/swap-shelf/`. Read the `shelf.csv` your scraper wrote, or call your scraper again from the
  pipeline. Either is fine. **Never copy the listings by hand.**
...
approval in writing, in your decision log, before the end of Thursday's Build 2.** A real website is **not**
...
1. **Ingest.** Read at least one messy source. Route A reads two: the listings and the sales log.
   - CSV is read with the `csv` module.
   - **If your pipeline scrapes the site live**, scraping follows every Lab U5-05 rule: `robots.txt` read and
     obeyed, `can_fetch` before every page, at least two seconds between requests, a User-Agent naming your
...
5. **Structures chosen on purpose.** Use at least one list, one dictionary, and one set, each with a comment
   saying why.
6. **Failures handled with a message, not a traceback:** the sales file missing and a required column missing.
   If your pipeline scrapes live, the site not running too.
...
8. **At least three test cases**, runnable with one command, covering at least one cleaning rule and at least one
...
data-pipeline/
  ...
  test_pipeline.py
  table_reader.py        only if your pipeline scrapes live, copied from the fixture folder
  data/                  your input files; for Route A, the sales log and your shelf.csv
...
3. **Data sources.** What each source is, and for Route A a section titled **Why this was allowed** quoting the
   site's `robots.txt` and the terms sentence that permits collecting the listings. Your Lab U5-05 README already
   has it; bring it over.
...
|---|---|---|
| **Define** | Week 11 Thu, Build 2 | Route chosen. The treasurer's two questions rewritten as numbers you will calculate, in `decision-log.md`. |
| **Measure** | Week 11 Thu, Build 2 | README section 4, **The mess**, from reading the data, before writing any cleaning code. |
| **Analyze** | Week 11 Thu, Build 2 | README section 5, **Rules I chose**, drafted. One calculated field worked by hand from three real rows. |
| **Improve** | Week 11 Thu to Week 11 Fri | Ingest Thursday. Clean, convert, and report Friday, in that order, committing after each stage works. |
| **Control** | Week 11 Fri, end of the pipeline block | Tests passing, README complete, the three-minute check given. **Due at the end of the pipeline block.** |
...
| Day | Block | Goal |
|---|---|---|
| Week 11 Wed | Flex | Optional, and only after your v3 is submitted. Read this spec and start thinking about your route. |
| Week 11 Thu | Build 2, first 25 minutes | Define, Measure, and Analyze. Route chosen and approved. The mess listed. Rules drafted. One field worked by hand. |
| Week 11 Thu | Build 2, last 15 minutes | Ingest: the listings and the sales log each read into a list of dictionaries, with row counts printed. |
| Week 11 Fri | Pipeline block, 30 minutes, after the quiz | Clean, convert to JSON, the report with calculated fields, three tests, the rest of the README, the three-minute check, final push. |

**This schedule is the tightest in the course.** It fits only because most of what you need already exists in your own
labs: the listings from U5-05, the cleaning pattern from U5-04, and the report pattern from U5-02. Reuse your code and
say so in the decision log. **Thursday's Measure is the step not to skip.** A student who writes the mess down first
writes each cleaning rule once on Friday. A student who skips it rewrites them.
...
> **Route A.** Reads the 28 listings from its Lab U5-05 `shelf.csv` and the sales log with `DictReader`. Normalizes item IDs
> so `oh-107` and `OH 109` match the listings. Refuses prices like `eight` and lists them with line numbers. Counts a
> duplicate sale once and says so. Writes `sales.json` and checks it reloads with the right count. The report shows,
> per platform, items listed, items sold, sell-through percent, and revenue, plus the average discount off the listed
> price, and lists five rows that need a person, including a sale typed with a letter O in the item ID. Four tests.
...
Week 11, Friday, in the last minutes of the pipeline block, in pairs. Your partner plays the treasurer. Then swap.

1. **One row that needs a person.** Show it in the report, then open the input file and show the line. Explain why
   your program refused to guess. (75 seconds)
2. **One number, checked.** Point at one calculated field and walk through the arithmetic from the raw data.
   (75 seconds)
3. **One question from the treasurer.** Your partner asks it; you answer it. (30 seconds)

**Steps 1 and 2 are the graded part of the check.** They are the two things the treasurer said mattered most. If your
pipeline scrapes live, have the site's terminal visible while it runs.
...
|---|---|---|
| **Functionality** | 25 | All four stages work on the real data. At least three calculated fields, one grouped. JSON written and reloaded. A missing sales file and a missing column each handled with a message, and the site being down too if you scrape live. |
| **Code Quality** | 20 | List, dictionary, and set each justified in a comment. Cleaning rules in dictionaries and sets, not long `if` chains. No bare `except`. If you scrape live, scraping follows every rule. |
| **Documentation** | 20 | All eight README sections. `Why this was allowed` quotes both documents. `Checked by hand` matches the report. |
| **Process** | 15 | DMAIC checkpoints on time, especially Measure before code. Three or more tests, one checked by hand. Commits after each stage. |
| **Demonstration** | 10 | The three-minute check, with a flagged row traced to its line and one number explained from raw data. |
...
**"The scraper is slow."** It is supposed to be. Three pages at two seconds each is about eight seconds. That is why
reading your saved `shelf.csv` is allowed: develop the cleaning code against it so you do not rescrape every run.
