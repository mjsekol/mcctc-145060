# serve_site.py
# Runs the Oak Hollow Game Club Swap Shelf, an invented club website, on
# your own computer so you can practise scraping without touching a real site.
#
# Usage, from this folder:
#     python serve_site.py            serves at http://127.0.0.1:8000/
#     python serve_site.py 8001       serves on a different port
#
# Press Ctrl+C in this terminal to stop it. Leave this terminal open while
# your scraper runs in a second terminal.
#
# You do not need to understand how this file works. It uses a class, and
# classes are Unit 7. What you need to know is what it does:
#
#   - It only answers requests from this computer (127.0.0.1).
#   - It prints one line for every request, with the time since the one
#     before, so you can SEE whether your scraper waits between requests.
#   - Like many real sites, it refuses requests that arrive less than one
#     second apart, with status 429 Too Many Requests.
...
class SwapShelfHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def check_rate(self):
        ...
        with SwapShelfHandler.lock:
            now = time.monotonic()
            first = SwapShelfHandler.last_request == 0.0
            gap = now - SwapShelfHandler.last_request
            SwapShelfHandler.last_request = now
        if first:
        ...
        if self.rate_limit and not first and gap < MINIMUM_GAP_SECONDS:
            ...
            if self.command != "HEAD":
                self.wfile.write(b"429 Too Many Requests. Wait at least two seconds between requests.\n")
            return False
        return True
...
def make_server(port, rate_limit=True):
    ...
    SwapShelfHandler.rate_limit = rate_limit
    SwapShelfHandler.last_request = 0.0
    handler = functools.partial(SwapShelfHandler, directory=str(SITE_FOLDER))
...
def main():
    ...
    try:
        ...
        return 1
    print(f"Swap Shelf running at http://{HOST}:{port}/")
